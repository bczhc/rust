...
impl Main {
    ...
    fn run(&mut self) -> Result<(), String> {
        let file_mode = u32::from_str_radix("644", 8).unwrap() as libc::mode_t;
        let dir_mode = u32::from_str_radix("755", 8).unwrap() as libc::mode_t;
        let executable_mode = u32::from_str_radix("755", 8).unwrap() as libc::mode_t;
        ...
        path.traversal_dir(|entry| unsafe {
            ...
            if file_type.is_file() {
                ...
                if libc::chmod(path_c_str_ptr, dir_mode) != 0 {
                    eprintln!(
                        "{}",
